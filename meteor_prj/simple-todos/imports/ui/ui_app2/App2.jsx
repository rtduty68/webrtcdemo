...
import {
  ...
  startConnection,
  startPeerConnection,
  reset as webRtcReset,
  restart as webRtcResart,
  onOffer,
  onAnswer,
  onCandidate
}
from '../../api/app2/client/webrtc.js'
import {
  callEventEnum,
  ActiveCallEvents,
  CurrentTime
}
from '../../api/app2/calls.js';
import {
  Random
}
from 'meteor/random'
...
class App2 extends Component {
  ...
  constructor(props) {
    ...
    Tracker.autorun(function() {
      if (Meteor.user() && Meteor.user().username) {
        console.log("subs activeCallevents " + Meteor.user().username);
    ...
    console.log("app2 construct name " + this.props.name);
    app2StoreInst.say();
    this.sessionId = null;
    ...
    this.callStateEnum = {
      ...
      PeerConnectionStart: "PeerConnectionStart"
    };
    this.state = {
      mystream: null,
      theirStream: null,
  ...
  componentDidMount() {
    ...
    Meteor.apply('getCurrentTime', [], {
      wait: true,
      noRetry: false
    }, getCurrentTimeResult.bind(this));
    ...
    Tracker.autorun(function() {
      var callEvents = ActiveCallEvents.find({}, {
        sort: {
          createdAt: -1
        }
      }).fetch();
      this.processCallEvents(callEvents);
  ...
  processEvent(event) {
    ...
    function onAnswerCreate(paramAnswer) {
      ...
      Meteor.apply('activeCallEvents.insert', [answer], {
        wait: true,
        noRetry: true
      }, insertResult);
    }
  ...
  makeCall() {
    function onOfferCreate(paramOffer) {
      ...
      Meteor.apply('activeCallEvents.insert', [offer], {
        wait: true,
        noRetry: true
      }, insertResult);
    }
    ...
    this.setState({
      callState: this.callStateEnum.PeerConnectionStart
    });
  ...
  dropCall() {
    ...
    Meteor.apply('activeCallEvents.insert', [leave], {
      wait: true,
      noRetry: true
    }, insertResult);
    this.onLeave();
  ...
  render() {
    console.log("render app2 page" + this.props.name);
  ...
  onicecandidate(paramCandidate) {
    ...
    Meteor.apply('activeCallEvents.insert', [candidate], {
      wait: true,
      noRetry: true
    }, insertResult);
  }
